    ...
    this.moveEnemy=function(){
        ...
        for(var i=0;i<_self.plane.length;i++){
            switch(_self.plane[i].className){
                case "small":
                    _self.plane[i].style.top=_self.plane[i].offsetTop+50+"px";
                    break;
                case "middle":
                    _self.plane[i].style.top=_self.plane[i].offsetTop+2+"px";
                    break;
                case "huge":
                    _self.plane[i].style.top=_self.plane[i].offsetTop+1+"px";
                    break;
            }
        }
        _self.timer=window.setTimeout(_self.moveEnemy,200);
    }
}