...
function query($mysql_server_name,$mysql_username,$mysql_password,$mysql_database,$sql){
    ...
    mysqli_query($conn,"set names 'utf8'");
    mysqli_query($conn,"set character set 'utf8'");
    $result=$conn->query($sql);//返回的是记录集  $result $conn 为内置变量 不需要声明
	$rs="";
    while($row=mysqli_fetch_assoc($result)){
		...
		$rs.="<td>".$row["p_sort"]."</td>";
		$rs.="<td>".$row["p_date"]."</td>";
		$rs.="<td><span class=\"".$row["ID"]."\">详情</span></td>";
		$rs.="<td><span class=\"".$row["ID"]."\">修改</span></td>";
		$rs.="<td><span class=\"".$row["ID"]."\">删除</span></td>";
		$rs.="</tr>";
    }
...
function doPost(){
    if($_POST["cmd"]!=NULL){
		...
		if($_POST["cmd"]=="2"){//插入数据  读取数据
			$f="p_name,p_subhead,p_price,p_discount,p_no,p_brand,p_brand_ID,p_dec,p_category,p_category_ID,p_category_path,p_saled,p_display,p_sort,p_date,p_pictures";
		    $v0="'".$_POST["p_name"]."',";
			$v1="'".$_POST["p_subhead"]."',";
			$v2="'".$_POST["p_price"]."',";
			$v3="'".$_POST["p_discount"]."',";
			...
			$v14="'".$_POST["p_date"]."',";
			$v15="''";
			$sql="insert into products(".$f.") values (".$v0.$v1.$v2.$v3.$v4.$v5.$v6.$v7.$v8.$v9.$v10.$v11.$v12.$v13.$v14.$v15.")";
		...
		if($_POST["cmd"]=="3"){
            ...
            //遍历request对象下的所有post|get方式请求的参数
            //$key:就是网页发送到服务器上的key名称，
            //$value:就是网页发送到到服务器上的key所对应的值
                if(array_key_exists($key,$_POST) && $key!="cmd" && $key!="identify"){
                    //array_key_exists($key,$_POST)这是php的内置方法，
                    //检查key是否存在于$_POST对象中
                    $sql.=$key."='".$value."',";
                }
            }
            $sql=substr($sql,0,strripos($sql,","));//字符串截取
            //strripos($sql,",")相当于js中lastIndexOf(",");
